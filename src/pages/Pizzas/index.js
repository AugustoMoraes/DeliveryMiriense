...
export default function Pizzas({route}){
    ...
    async function loadingList(){
      ...
      await firebase.database().ref(item.key).child('pizza').orderByChild('nome').on('value', (snapshot)=> {
        setProdutos([])
    ...
    async function loadingListSucos(){
      ...
      await firebase.database().ref(item.key).child('sucos').orderByChild('nome').on('value', (snapshot)=> {
        setSucos([])
        snapshot.forEach((childItem) => {
          let list = {
            key: childItem.key,
    ...
    async function loadingListRefri(){
      ...
      await firebase.database().ref(item.key).child('refrigerantes').orderByChild('nome').on('value', (snapshot)=> {
        setRefri([])
        snapshot.forEach((childItem) => {
          let list = {
            key: childItem.key,
